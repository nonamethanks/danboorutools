...
class CircleMsParser(UrlParser):
    ...
    @classmethod
    def match_url(cls, parsable_url: ParsableUrl) -> CircleMsCircleUrl | UselessUrl | None:  # type: ignore[return]
        match parsable_url.url_parts:
            ...
            case ("Circle" | "circle"), ("Index" | "index"), circle_id if parsable_url.subdomain == "portal":
                return CircleMsCircleUrl(parsed_url=parsable_url,
            ...
            case *_, asp_page if asp_page.endswith(".aspx") and cls.id_subdomain.match(parsable_url.subdomain):
                return CircleMsCircleUrl(parsed_url=parsable_url,
                                         circle_id=int(parsable_url.subdomain.removeprefix("c")))
