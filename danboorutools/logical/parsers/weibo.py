...
from danboorutools.logical.urls import weibo as w
...
class WeiboComParser(UrlParser):
    ...
    @classmethod
    def match_url(cls, parsable_url: ParsableUrl) -> w.WeiboUrl | None:  # type: ignore[return]
        match parsable_url.url_parts:
            ...
            case artist_id, illust_base62_id if artist_id.isnumeric() and not illust_base62_id.islower():
                # dumb assumption, will bite me in the ass later
                return w.WeiboPostUrl(parsed_url=parsable_url,
            ...
            case artist_id, _, _, _, illust_long_id, *_ if parsable_url.subdomain == "photo":
                return w.WeiboPostUrl(parsed_url=parsable_url, illust_long_id=int(
                    illust_long_id.split("#")[0]), artist_id=int(artist_id))
            ...
            case ("u" | "profile"), artist_id, *_ if artist_id.isnumeric():
                return w.WeiboArtistUrl(parsed_url=parsable_url,
                                        artist_id=int(artist_id))
            ...
            # https://www.weibo.com/p/1006065770760941/photos?from=page_100606#wbphoto_nav
            case "p", artist_long_id, *_ if artist_long_id.startswith(("100505", "100606")):
                return w.WeiboArtistUrl(parsed_url=parsable_url,
                                        artist_id=int(artist_long_id[6:]))

            # https://www.weibo.com/5501756072
            # https://www.weibo.cn/5501756072
            # https://weibo.com/1843267214/profile
            case artist_id, *_ if artist_id.isnumeric():
                return w.WeiboArtistUrl(parsed_url=parsable_url,
                                        artist_id=int(artist_id))
            ...
            case "n", screen_name, *_rest:
                return w.WeiboLegacyArtistUrl(parsed_url=parsable_url,
...
class SinaimgCnParser(UrlParser):
    ...
    @classmethod
    def match_url(cls, parsable_url: ParsableUrl) -> w.WeiboImageUrl | None:  # type: ignore[return]
        match parsable_url.url_parts:
            ...
            case dimensions, _ if dimensions.startswith(cls.SIZE_STRINGS):
                return w.WeiboImageUrl(parsed_url=parsable_url)
