...
class BoothPmParser(UrlParser):
    ...
    @staticmethod
    def _match_everything_else(parsable_url: ParsableUrl) -> b.BoothUrl | None:
        instance: b.BoothUrl
        match parsable_url.url_parts:
            ...
            case *_, "i", item_id, _:
                instance = b.BoothImageUrl(parsable_url)
                instance.item_id = int(item_id)
            ...
            case _, _ if parsable_url.subdomain in ["s2", "s"]:
                instance = b.BoothProfileImageUrl(parsable_url)
                instance.user_id = None
