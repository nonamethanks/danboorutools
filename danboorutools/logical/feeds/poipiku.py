...
class PoipikuFeed(Feed):
    ...
    def _extract_posts_from_each_page(self) -> Iterator[list[tuple[PoipikuPostUrl, list[PoipikuImageUrl]]]]:
        ...
        while True:
            ...
            for post_element in self.session.browser.find_elements("css selector", ".IllustItem"):
                ...
                if (expand := post_element.find_elements("css selector", ".IllustItemExpandBtn")):
                    self.session.browser.execute_script("arguments[0].click()", expand[0])
                    self.session.browser.wait_for_request("ShowAppendFileF", timeout=60)
                    time.sleep(1)
