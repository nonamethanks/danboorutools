...
def assert_url(url: str,
    ...
    assert parsed_url.is_deleted == is_deleted
    return parsed_url
...
def assert_asset_url(url: str,
    ...
    assert len(asset.files) == file_count

    if created_at:
        assert asset.created_at == datetime_from_string(created_at)
